...
var Instances = require('settings-instances');
...
page("/settings/:page?", valid, user.required, function(ctx, next) {
  if (!ctx.valid) {
    return next();
  }
...
function valid(ctx, next) {
  var page = ctx.params.page || "profile";
  var valid = ['profile', 'password', 'notifications', 'instances'];
