...
var log = require('debug')('hub:form-view');
...
function FormView(template, options) {
  if (!(this instanceof FormView)) return inherit(template, FormView);
...
FormView.prototype.response = function(err, res) {
  if (err) {
    return this.emit('error'), this.errors([err]);
  }
  if (res.body && res.body.error) {
    return this.emit('error'), this.errors([res.body.error]);
  }
  if (!res.ok) {
    return this.emit('error'), this.errors([res.text]);
  }
