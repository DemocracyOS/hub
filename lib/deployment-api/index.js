...
app.get('/mine', restrict, function(req, res, next) {
  api.deployment.findByOwner(req.user.id, function(err, deployments) {
    if (err) return _handleError(err, req, res);
...
app.post('/', restrict, maintenance, function(req, res, next) {
  var data = {
...
app.delete('/:id', restrict, maintenance, function(req, res, next) {
