...
exports.secureurl = function(val, option, fn) {
  fn('https' != val.substring(0, 5) ? t('validators.invalid.secureurl') : null);
};
...
exports.username = function(val, option, fn) {
  val = val.trim();
  fn(!regexps.username.test(val) && '' != val ? t('validators.invalid.username') : null);
};
...
exports.subdomain = function(val, option, fn) {
  fn('' != val.trim() && !regexps.subdomain.test(val) ? t('validators.invalid.subdomain') : null);
};
